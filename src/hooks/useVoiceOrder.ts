...
export const useVoiceOrder = ({ products }: UseVoiceOrderProps) => {
  ...
  const transcribeWithBrowserAPI = useCallback((audioBlob: Blob): Promise<string> => {
    return new Promise((resolve, reject) => {
      // Use Web Speech API for transcription (browser-based, no server needed)
      const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
      if (!SpeechRecognition) {
        reject(new Error("Speech recognition not supported in this browser"));
        return;
      }
      // Since we already have the audio, we'll use a different approach
      // We'll use the speech recognition API directly during recording instead
      reject(new Error("use_live_recognition"));
    });
  }, []);
